...
public class UserProfileComponentTest {
  ...
  public void should_retrieve_a_user() {
    ...
    StepVerifier.create(actualUser).expectNext(user0).verifyComplete();
  }
  ...
  public void should_create_a_new_user() {
    ...
    when(userProfileRepository.findUserByUsername(eq(username))).thenReturn(Mono.empty());

    // When mario.rossi tries to register
    Long mockedId = 0L;
    UserProfileEntity entity =
        UserProfileEntity.builder().userName(username).name(name).surname(surname).build();
  ...
  public void should_not_create_new_user_because_already_exists() {
    ...
    UserProfileEntity entity =
        UserProfileEntity.builder()
            ...
            .surname(surname)
            .build();
    when(userProfileRepository.findUserByUsername(eq(username))).thenReturn(Mono.just(entity));
  ...
  public void should_update_name_and_surname_of_existing_user() {
    ...
    UserProfileEntity newData =
        UserProfileEntity.builder()
            ...
            .build();
    when(userProfileRepository.saveUser(eq(newData))).thenReturn(Mono.just(newData));

    // Then the data are modified and the new one are returned
    var result = userProfileComponent.updateUser(newData);
    StepVerifier.create(result).expectNext(UserProfileEntity.copy(newData)).verifyComplete();
  }

  @Test
  public void should__fail_to_update_a_non_existing_user() {
    // Given an unexisting user
    when(userProfileRepository.findUserById(eq(0L))).thenReturn(Mono.empty());
