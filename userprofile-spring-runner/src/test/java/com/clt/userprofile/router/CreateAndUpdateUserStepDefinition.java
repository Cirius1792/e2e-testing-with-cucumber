...
public class CreateAndUpdateUserStepDefinition {
  ...
  public void a_new_user_that_wants_to_register() {
    user = UserProfileEntity.builder().userName(UUID.randomUUID().toString()).build();
  }
  ...
  public void the_user_has_not_a_unique_username() {
    ...
    var driver = new UserApplicationDriver(client);
    driver.createUser(
        Map.of(UserApplicationDriver.UserParametersEnum.USERNAME, user.getUserName()));
  }
  ...
  public void the_outcome_of_the_operation_of_is(String outcome) {
    if ("OK".equals(outcome)) Assertions.assertTrue(this.result.getStatus().is2xxSuccessful());
    else Assertions.assertTrue(this.result.getStatus().isError());
  }
